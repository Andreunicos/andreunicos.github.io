/* Onde exatamente se foram os milissegundos do laço de estatísticas. */
...
async function principal() {
  ...
  try {
    ...
    const r = await A.evaluate(async () => {
      const cron = async (f, n) => {
        const t = [];
        for (let i = 0; i < n; i++) { const a = performance.now(); await f(); t.push(performance.now() - a); }
        return +(t.reduce((x, y) => x + y, 0) / t.length).toFixed(3);
      };
      const p = [...pares.values()][0];
      return {
        tudo:        await cron(() => atualizarNumeros(), 40),
        semTelaFalsa: await (async()=>{ const ids=[]; for(let i=1;i<9999;i++) ids.push(i); ids.forEach(i=>clearInterval(i)); return cron(() => atualizarNumeros(), 40); })(),
        soGetStats:  await cron(() => p.pc.getStats(), 40),
        soPerfilReal:await cron(() => { perfilReal(); }, 40),
        soCaptura:   await cron(() => { capturaSuspeita(est.histFonte || [], 60); }, 40),
        soPacing:    await cron(() => { pacingDe(p); }, 40),
        soPrio:      await cron(() => arrumarPrioridades(p), 40),
        soAnotar:    await cron(() => { try { anotarLinha({ par: p, ms: 1, perda: 0, kbps: 0 }); } catch (e) {} }, 40),
      };
    });
    console.log('\n=== ONDE SE FOI O TEMPO (média por chamada, ms) ===');
    Object.entries(r).sort((a, b) => b[1] - a[1])
      .forEach(([k, v]) => console.log('  ' + k.padEnd(14) + String(v).padStart(8) + ' ms'));
    console.log('\n  getStats sozinho e ' + (r.soGetStats / r.tudo * 100).toFixed(0) +
                '% do laco inteiro.');
